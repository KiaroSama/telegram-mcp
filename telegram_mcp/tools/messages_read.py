"""Reading messages back out of Telegram: history, context and pins.
...
conversation around one message (``get_message_context``), or listing what is
pinned (``get_pinned_messages``). Searching moved to ``message_search``. ``mark_as_read`` moved to
