"""Media inspection: what a message's media IS, and what it LOOKS like.

Split from ``inspection.py``, which had grown to hold two jobs that share only a
message lookup. The tools here all turn bytes into a picture - a thumbnail, video
frames, a rendered Lottie emoji - and carry every heavy dependency that implies.
``inspection.py`` keeps the ones that answer what the API SAYS about a message and
touch no image data at all.

The dependency runs one way, ``media_inspection`` -> ``inspection``, for the
message lookup and the account guard. Nothing here is imported back.
"""

# The MODULE, not the names: `_get_message` is reached as `inspection._get_message`
# so a test patching it on its own module reaches this half too. `from ... import
# _get_message` would bind the function object here at import time, and the patch
# would keep succeeding against a name nothing reads - the exact trap the last
# split in this repository fell into.
from telegram_mcp.tools import inspection
from telegram_mcp.tools.inspection import DEFAULT_THUMBNAIL_BYTES, require_explicit_account
...
async def get_media_details(chat_id: Union[int, str], message_id: int, account: str = None) -> str:
    ...
    """
    try:
        _, _, msg = await inspection._get_message(chat_id, message_id, account)
        if not msg:
...
async def get_media_thumbnail(
    ...
    """
    try:
        cl, _, msg = await inspection._get_message(chat_id, message_id, account)
        if not msg:
            return f"Message {message_id} was not found in chat {chat_id}."
        ...
        async def _refetch_message():
            _, _, refreshed = await inspection._get_message(chat_id, message_id, account)
