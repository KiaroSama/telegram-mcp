...
async def edit_scheduled_message(
    ...
    try:
        ...
        if message is not None and entities is not None:
            ...
            # The text is carried over below; its FORMATTING has to be carried
            # with it. `EditMessage` replaces the message wholesale, so resending
            # the same characters with no entities strips them - a premium emoji
            # became a plain character, a link became text, because someone
            # changed the delivery time.
            #
            # Not carried when new `message` text IS supplied: those entities
            # index the OLD string, so keeping them would place formatting at
            # offsets that no longer mean anything.
            built_entities = getattr(current, "entities", None) or None
