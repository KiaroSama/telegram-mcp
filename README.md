...
- **Admin rights newer than Telethon's layer:** Telegram silently drops flags added after the TL layer a client announces — measured, not assumed: one request from a channel's own creator carrying flags.18, flags.19 and flags.20 was accepted and only flags.18 survived. Telethon announces 227 and is archived, so `edit_admin_rights` now reports the rights it could not deliver instead of claiming success, and `set_admin_right` puts one of them on the wire through TDLib (layer 229), preserving every other right by reading and writing the whole rights object inside TDLib rather than translating names between the two libraries.
