...
def test_scan_settled_picks_quiet_chat():
    now = time.monotonic()
    events_store._pending_msgs[(ACCOUNT, 1)] = _pending_record(now - 10)
    events_store._pending_msgs[(ACCOUNT, 2)] = _pending_record(now)

    settled, soonest = events._scan_settled(now, settle=6.0)
    assert settled == (ACCOUNT, 1)

    del events_store._pending_msgs[(ACCOUNT, 1)]
    settled, soonest = events._scan_settled(now, settle=6.0)
...
async def test_feed_writes_settled_burst_and_consumes_it():
    ...
    for _ in range(50):
        ...
        if not events_store._pending_msgs:
            break

    assert events_store._pending_msgs == {}
    lines = events_store.feed_file_path().read_text(encoding="utf-8").strip().splitlines()
    assert len(lines) == 1
...
async def test_enable_disable_and_status_tools():
    ...
    assert events_store.feed_file_path().exists()
...
async def test_wait_for_settled_message_still_works():
    events_store._pending_msgs[(ACCOUNT, 7)] = _pending_record(_mono(10))
...
async def test_enable_with_unwritable_path_starts_nothing(monkeypatch, tmp_path):
    ...
    assert (
        ACCOUNT,
        42,
    ) in events_store._pending_msgs  # burst still available to wait_for_settled_message
...
async def test_write_failure_retains_burst(monkeypatch, tmp_path):
    ...
    await asyncio.sleep(0.3)
    assert (ACCOUNT, 42) in events_store._pending_msgs  # not silently destroyed
...
def test_default_feed_path_is_runtime_state_not_install_dir(monkeypatch, tmp_path):
    ...
    assert Path(events_store.__file__).parent not in path.parents
...
def test_feed_file_created_owner_only():
    events_store._touch_feed_file()
    mode = stat.S_IMODE(events_store.feed_file_path().stat().st_mode)
    assert mode == 0o600
...
async def test_wait_for_chat_ignores_other_chats(monkeypatch):
    ...
    # noise from someone else
    events_store._pending_msgs[(ACCOUNT, 999)] = _pending_record(_mono(10))
    ...
    assert (ACCOUNT, 999) in events_store._pending_msgs  # and its burst is still there for later
...
async def test_wait_for_chat_returns_when_that_chat_speaks(monkeypatch):
    monkeypatch.setattr(events, "_wait_target", _target(42))
    events_store._pending_msgs[(ACCOUNT, 999)] = _pending_record(_mono(10))
    events_store._pending_msgs[(ACCOUNT, 42)] = _pending_record(_mono(10))
    ...
    assert (ACCOUNT, 999) in events_store._pending_msgs  # unrelated burst untouched


@pytest.mark.asyncio
async def test_wait_for_new_message_filters_by_chat(monkeypatch):
    monkeypatch.setattr(events, "_wait_target", _target(42))
    events_store._pending_msgs[(ACCOUNT, 999)] = _pending_record(_mono(1))

    timed_out = json.loads(await events.wait_for_new_message(timeout=0.2, chat_id=42))
    assert timed_out["event"] is False

    events_store._pending_msgs[(ACCOUNT, 42)] = _pending_record(_mono(1))
...
async def test_two_accounts_in_the_same_chat_id_do_not_merge(monkeypatch):
    ...
    assert len(events_store._pending_msgs) == 2, (
        "the two accounts collapsed into one record: " f"{events_store._pending_msgs}"
    )
    for key, record in events_store._pending_msgs.items():
        assert record["count"] == 1, f"{key} absorbed the other account's message"
        assert record["account"] in ("work", "personal")
    accounts = {record["account"] for record in events_store._pending_msgs.values()}
...
async def test_a_burst_is_reported_with_the_account_it_arrived_on(monkeypatch):
    ...
    ((key, record),) = events_store._pending_msgs.items()
...
async def _queue_burst(chat_id, tries=60):
    ...
    """
    events_store._pending_msgs[(ACCOUNT, chat_id)] = _pending_record(_mono(1.0))
...
async def test_the_feed_rotates_instead_of_growing_without_end(monkeypatch):
    ...
    live = events_store.feed_file_path()
...
def test_a_retention_budget_that_is_not_a_budget_falls_back(monkeypatch, value):
    ...
    max_bytes, max_age = events_store.feed_retention()

    assert max_bytes == events_store._FEED_MAX_BYTES_DEFAULT
    assert max_age == events_store._FEED_MAX_AGE_SECONDS_DEFAULT
...
def test_a_pending_bound_that_is_not_a_bound_falls_back(monkeypatch, value):
    ...
    max_count, ttl = events_store.pending_bounds()

    assert max_count == events_store._PENDING_MAX_DEFAULT
...
async def test_wait_for_new_message_bounds_the_list_it_serializes(monkeypatch):
    ...
    for chat in range(300):
        events_store._pending_msgs[(ACCOUNT, chat)] = _pending_record(_mono(1))
...
async def test_wait_for_new_message_refuses_a_limit_that_is_not_one(monkeypatch):
    events_store._pending_msgs[(ACCOUNT, 1)] = _pending_record(_mono(1))
...
def test_a_created_feed_file_is_readable_only_by_its_owner():
    ...
    assert _owner_only(events_store.feed_file_path())
...
def test_every_file_the_feed_creates_goes_through_the_restriction_seam(monkeypatch):
    ...
    seen = []
    monkeypatch.setattr(
        events_store, "_restrict_to_owner", lambda path, fd=None: seen.append(str(path))
    )
    monkeypatch.setenv("TELEGRAM_EVENT_FEED_MAX_BYTES", "1")

    events_store._touch_feed_file()
    live = str(events_store.feed_file_path())
    assert seen == [live]

    events_store.feed_file_path().write_text("x" * 64, encoding="utf-8")
    events_store._touch_feed_file()  # over the ceiling, so this one rotates
